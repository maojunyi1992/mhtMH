...
class Agent extends BaseController
{
    ...
    public function list_table()
    {
		...
		foreach($getAgentList['rows'] as $key=>$val){
			...
			$val['all_money'] = '<b>包含下级：'.$money.'元</b><br>不含下级：'.$myMoney.'元';
			$val['today_money'] = '<b>包含下级：'.$todayMoney.'元</b><br>不含下级：'.$myTodayMoney.'元';
			$val['lastday_money'] = '<b>包含下级：'.$lastMoney.'元</b><br>不含下级：'.$myLastMoney.'元';
    ...
    public function addSubmit()
    {
		...
		if(isset($post['fencheng']) && $post['fencheng']!=null){
			...
			if($fencheng > $this->myAdmin['fencheng'] || $fencheng < 0 ){
