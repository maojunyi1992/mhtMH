...
function ZhuanZhiBaoShi:RefreshFormInfo()
	self:RefreshAllGemData()
	self.m_ChaJia:setText("0")
	local curNum = CurrencyManager.getOwnCurrencyMount(fire.pb.game.MoneyType.MoneyType_SilverCoin)
...
function ZhuanZhiBaoShi:CalculateChaJia()
	...
	local Price2 = ShopManager.goodsPrices[BaoShiShopStartID[idx2]] or conf2.prices[0]
	    local itemAttrCfg1 = BeanConfigManager.getInstance():GetTableByName("item.citemattr"):getRecorder(self.m_NextGemID)
    if not itemAttrCfg1 then
        return
    end
	...
	Price2 = Price2 * math.pow(2,self.m_CurGemQulity - 1)
	  local gemLevel = itemAttrCfg1.level
	local value = m_baoshixiaohao * gemLevel
