...
function MainControl:OnCreate()
	...
	self.m_pPackBtn1 = CEGUI.Window.toPushButton(winMgr:getWindow("MainControlDlg/back/pack1"))
    ...
    self.m_pZhuzhanBtn = CEGUI.Window.toPushButton(winMgr:getWindow("MainControlDlg/back/zhuzhan"))
    self.m_pZhuzhanBtn:subscribeEvent("MouseClick", MainControl.HandleZhuzhanBtnClicked, self)
	
	self.m_zuoqi = CEGUI.Window.toPushButton(winMgr:getWindow("MainControlDlg/back/pack2"))
	
	self.m_zuoqi:setVisible(false)
    self.m_pPackBtn1:setVisible(false)
	...
	self.m_pPackBtn1:subscribeEvent("MouseClick", MainControl.HandlePackBtnClicked1, self)
    ...
    self.m_pPaihangBtn:subscribeEvent("Clicked", MainControl.HandlePaiHangBtnClicked, self)
     self.m_zuoqi:subscribeEvent("MouseClick", MainControl.HandlePackBtnClicked2, self)
    ...
    local zhuzhanStat = { }
    zhuzhanStat.pWnd = self.m_pZhuzhanBtn
    zhuzhanStat.bUnlocked = true
    self.m_aDownWndSt[eZhuzhanPos] = zhuzhanStat
...
function MainControl:initIsSimplify()
    ...
    if record.id ~= -1 then
        ...
        if value == 1 then 
			...
			self.m_pPackBtn1:setVisible(true)
            self.m_SimplifyBtn:setVisible(false)
			self.m_zuoqi:setVisible(true)	
            self:UnFoldButton()
...
function MainControl:HandlePackBtnClicked1(args)
    require("logic.pointcardserver.vipfuliditudlg4").getInstanceAndShow()
end

function MainControl:HandlePackBtnClicked2(args) --坐骑
    debugrequire "logic.zuoqi.characterzuoqidlg".getInstanceAndShow()
end
...
function MainControl:HandleZhuzhanBtnClicked(args)
    LogInfo("maincontrol xiake btn clicked")
    -- self.m_fAutoHideTime = 0
...
function MainControl:InitBtnShowStat()
    ...
    if curLevel >= beanTabel:getRecorder(6).needlevel then
        self.m_aDownWndSt[eZhuzhanPos].bUnlocked = true
    end
   --帮派
...
function MainControl:GetButtonPos(wnd)
    if wnd == self.m_pSkillBtn or wnd == self.m_pTaskBtn or wnd == self.m_pFriendBtn or wnd == self.m_pXiakeBtn or wnd == self.m_pZhuzhanBtn or wnd == self.m_pPaihangBtn or wnd == self.m_pTeamBtn or wnd == self.m_pJewelryBtn then
        return 0
...
function MainControl:IsInMainControl(pWnd)
    if pWnd == self.m_pSkillBtn or pWnd == self.m_pFriendBtn or pWnd == self.m_pXiakeBtn or pWnd == self.m_pZhuzhanBtn or pWnd == self.m_pProductBtn or pWnd == self.m_pTeamBtn or pWnd == self.m_pPaihangBtn or pWnd == self.m_pPackBtn or pWnd == self.m_pTaskBtn or pWnd == self.m_pJewelryBtn then
        return true
    end
    return false
end
