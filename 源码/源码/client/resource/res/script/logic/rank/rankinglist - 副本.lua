-- @Author: 作者QQ995761937
-- @Date:   2022-06-21 16:16:10
-- @Last Modified by:   作者QQ995761937
-- @Last Modified time: 2022-06-22 21:48:17
    ...
    if _instance then
        ...
        if myrank == 0 then
            ...
            elseif ranktype == typeEnum.PROFESSION_WARRIOR_RANK or
                ...
                ranktype == typeEnum.PROFESSION_WARLOCK_RANK or
                ranktype == 63 or
                ranktype == 64 or
                ranktype == 65
                then
        ...
        if ranktype == typeEnum.LEVEL_RANK then
            ...
            for k = 1, sizeof_recordlist do
                ...
                row:unmarshal(_os_)
                _os_:delete()
                local zhuan = gGetDataManager():GetMainCharacterForm()
                _instance:AddRow(row.rank - 1, row.roleid, tostring(row.rank), row.nickname, BeanConfigManager.getInstance():GetTableByName("role.schoolinfo"):getRecorder(row.school).name, tostring(row.form).."转"..tostring(row.level))
            end
            ...
            _instance:UpdateMyRank(tostring(myrank), myname, myschool, tostring(zhuan.."转"..mylevel));
            -- 更新自己的信息 by changhao
        ...
        elseif ranktype == typeEnum.FACTION_RANK then
            ...
            for k = 1, sizeof_recordlist do
                ...
                row:unmarshal(_os_)
                _os_:delete()
                _instance:AddRow(row.rank - 1, row.factionkey, tostring(row.rank), row.factionname, tostring(zhuan.."转"..row.level), row.mastername)
            end
        elseif ranktype == typeEnum.ROLE_ZONGHE_RANK then
            -- 综合排名 by changhao
            local sizeof_recordlist = #list
            for k = 1, sizeof_recordlist do
                ...
                row:unmarshal(_os_)
                _os_:delete()
                _instance:AddRow(row.rank - 1, row.roleid, tostring(row.rank), row.rolename, BeanConfigManager.getInstance():GetTableByName("role.schoolinfo"):getRecorder(row.school).name,tostring(row.score))
            end
            ...
            local mylevel = gGetDataManager():GetMainCharacterLevel()
            local zhuan = gGetDataManager():GetMainCharacterForm()
            _instance:UpdateMyRank(tostring(myrank), myname, myschool, tostring(zhuan.."转"..mylevel), tostring(extdata));
            -- 更新自己的信息 by changhao			
        ...
        elseif ranktype == typeEnum.PROFESSION_WARRIOR_RANK or
            ...
            for k = 1, sizeof_recordlist do
                ...
                row:unmarshal(_os_)
                _os_:delete()
                _instance:AddRow(row.rank - 1, row.roleid, tostring(row.rank), row.rolename, tostring(zhuan.."转"..row.rolelevel),tostring(row.score))
            end
            ...
            local mylevel = gGetDataManager():GetMainCharacterLevel()
            local zhuan = gGetDataManager():GetMainCharacterForm()
            _instance:UpdateMyRank(tostring(myrank), myname, tostring(zhuan.."转"..mylevel), tostring(extdata));
            -- 更新自己的信息 by changhao
...
function RankingList:InitTree()
    ...
    record = BeanConfigManager.getInstance():GetTableByName("game.cpaihangbang"):getRecorder(101)
    -- 宠物 by changhao
    second = first:addItem(CEGUI.String(record.leixing), 101)
    SetGroupBtnTreeSecondIcon(second)
