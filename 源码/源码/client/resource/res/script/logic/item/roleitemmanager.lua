...
function RoleItemManager:GetItemByLoction(bagid, location)
    ...
    for first, second in pairs(list) do
        local pItem = second
        if pItem then
            if pItem:GetLocation().position == location then
                return pItem,first
            end
        end
    end
...
function RoleItemManager:setfenbang(name)
    self.m_fenbang = name
end

function RoleItemManager:getfenbang()
    return self.m_fenbang
end
...
function RoleItemManager:GetWeaponList(bagid)
    -- Ĭ��ֵ
    if not bagid then
        bagid = fire.pb.item.BagTypes.BAG
    end
	...
	if self.m_BagInfo[bagid] then
		for first, second in pairs(self.m_BagInfo[bagid]) do
			...
			if pItorItem then
				local typeid=pItorItem:GetItemTypeID()
				  if  typeid==1288 or typeid==3080 or typeid==4360 or typeid==1800 or typeid==2824 or typeid==4104 then
