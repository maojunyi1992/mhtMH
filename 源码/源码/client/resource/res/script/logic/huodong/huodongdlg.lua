...
function HuodongDlg:refreshActiveGift()
	...
	for _, v in pairs(indexIDs) do
		...
		elseif index == 1 then
			...
			local x = CEGUI.UDim(0, 1 +  math.floor(winWidth) * record.needactvalue / macactvalue + 410)
			local x1 = CEGUI.UDim(0, 1 +  math.floor(winWidth) * record.needactvalue / macactvalue + 410)
		...
		elseif index == 3 then
			...
			local x = CEGUI.UDim(0, 1 +  math.floor(winWidth) * record.needactvalue / macactvalue + 350)
			local x1 = CEGUI.UDim(0, 1 +  math.floor(winWidth) * record.needactvalue / macactvalue + 350)
		...
		elseif index == 4 then
			...
			local x = CEGUI.UDim(0, 1 +  math.floor(winWidth) * record.needactvalue / macactvalue + 320)
			local x1 = CEGUI.UDim(0, 1 +  math.floor(winWidth) * record.needactvalue / macactvalue + 320)
		...
		elseif index == 5 then
			...
			local x = CEGUI.UDim(0, 1 +  math.floor(winWidth) * record.needactvalue / macactvalue + 290)
			local x1 = CEGUI.UDim(0, 1 +  math.floor(winWidth) * record.needactvalue / macactvalue + 290)
...
function HuodongDlg:SortTable()
	...
	for i, v in pairs(self.m_cells) do
		local x = CEGUI.UDim(0, 1 + ((i - 1) % 2) * v.m_width)
		local y = CEGUI.UDim(0, 1 + math.floor((i - 1) / 2) * v.m_height)
		local pos = CEGUI.UVector2(x, y)
		v:GetWindow():setPosition(pos)
	
		print("Cell " .. i .. " position: x = " .. pos.x.scale * v:GetWindow():getParentPixelSize().width .. ", y = " .. pos.y.scale * v:GetWindow():getParentPixelSize().height)
	end
